...
local updateHealCommBar = function(frame, unit)
	...
	if(healCommBar.Amount)then
		oUF_HealComm:SetFontType(frame,healCommBar.Amount)
...
local hook = function(frame)
	...
	oUF_HealComm:SetFontType(frame,hcb.Amount)	
	hcb.Amount:SetPoint('BOTTOM',hcb,"CENTER", 0, 0)
	hcb.Amount:SetTextColor(1,1,1,.5)
	hcb.Amount:SetJustifyH("CENTER")
	hcb.Amount:SetJustifyV("MIDDLE")
	...
	frame.line:SetTexture(healLineTexture)
...
function oUF_HealComm:SetFontType(frame,textObject)

	if(frame.FontObjects and  frame.FontObjects.health)then
		fontObj = frame.FontObjects.health.object
	elseif(frame.Health and frame.Health.text)then
		fontObj = frame.Health.text
	elseif(frame.HealthText)then
		fontObj = frame.HealthText
	end
	
	if(fontObj)then
		fontName, fontHeight, fontFlags = fontObj:GetFont()
	else
		fontName, fontHeight, fontFlags = font, fontSize, "outline"
	end
	
	textObject:SetFont(fontName, fontHeight, fontFlags)
end
